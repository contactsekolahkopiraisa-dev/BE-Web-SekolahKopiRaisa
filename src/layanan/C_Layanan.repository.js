...
const konfigurasiLayananRepository = {
  findByHashAndJenis: async (hash, idJenisLayanan) =>
    prisma.KonfigurasiLayanan.findFirst({
      where: {
  ...
  create: async (idJenis, hash, isActive, listDetail) => {
    return prisma.$transaction(async (tx) => {
      ...
      let urutan = 1;
      const detailRows = listDetail.flatMap((item) =>
...
const layananRejectionRepository = {
  update: async (id, data) => {
    return prisma.layananRejection.update({
...
const jenisLayananRepository = {
  ...
  findById: async (id) =>
    await prisma.JenisLayanan.findUnique({
      where: { id: parseInt(id) },
      include: {
        targetPeserta: true,
      },
  ...
  update: async (id, safeData) =>
    prisma.JenisLayanan.update({
      where: { id: parseInt(id) },
      data: safeData,
    }),
};
...
const targetPesertaRepository = {
  ...
  findById: async (id) =>
    await prisma.TargetPeserta.findUnique({
      where: { id: parseInt(id) },
    }),
};
...
const statusKodeRepository = {
  ...
  findById: async (id) =>
    await prisma.StatusKode.findUnique({
      where: { id: parseInt(id) },
    }),
  findByName: async (name) => {
