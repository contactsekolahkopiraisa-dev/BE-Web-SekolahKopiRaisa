...
const authMiddleware = async (req, res, next) => {
  ...
  try {
    ...
    const err = new Error("*Token invalid atau sudah kadaluwarsa!");
    err.statusCode = 401;
    return next(err);
  }
};
...
const validateInsertNewsMedia = (req, res, next) => {
  ...
  if (!thumbnailFile) {
    ...
    if (!allowedTypes.includes(thumbnailFile.mimetype)) {
      req.mediaValidationErrors.thumbnail =
        "*Sampul hanya boleh berupa gambar (jpg, jpeg, png, webp)";
  ...
  if (mediaFiles.length > 0) {
    ...
    if (invalidFiles.length > 0) {
      req.mediaValidationErrors.media =
    ...
    if (totalSize > maxTotalSize) {
      req.mediaValidationErrors.media =
        "*Total ukuran file tidak boleh lebih dari 25MB";
    }
  }
...
const validateUpdateNewsMedia = (options = {}) => {
  return (req, res, next) => {
    ...
    const invalidMediaFiles = mediaFiles.filter(
      (file) => !allowedTypes.includes(file.mimetype)
    );
    ...
    const oversizedMediaFiles = mediaFiles.filter(
      (file) => file.size > maxSizeBytes
    );
...
const validateProductMedia = (req, res, next) => {
  ...
  if (!mediaFile || mediaFile === undefined) {
    ...
    if (!allowedTypes.includes(mediaFile.mimetype)) {
      req.mediaValidationErrors.productFile =
        "*Gambar produk hanya boleh berupa gambar (jpg, jpeg, png, webp)";
...
const validateAboutCompanyMedia = (req, res, next) => {
  ...
  if (!mediaFile) {
    ...
    if (!allowedTypes.includes(mediaFile.mimetype)) {
      req.mediaValidationErrors[fieldName] =
        "*Gambar perusahaan hanya boleh berupa (jpg, jpeg, png, webp)";
    }
    // 3. Jika tipe file benar, validasi ukurannya
    else if (mediaFile.size > maxSizeBytes) {
      req.mediaValidationErrors[
        fieldName
      ] = `*Ukuran gambar perusahaan maksimal ${maxSizeMB}MB`;
    }
  }
...
const multerErrorHandler = (err, req, res, next) => {
  ...
  if (
    ...
    err.message.startsWith("File type not allowed for field:")
  ) {
  ...
  if (
    !req.file &&
    !req.files &&
    (req.baseUrl.includes("product") || req.baseUrl.includes("news"))
  ) {
    const fallbackField = req.baseUrl.includes("news")
      ? "media"
