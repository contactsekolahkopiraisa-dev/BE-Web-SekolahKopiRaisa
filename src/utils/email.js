...
function createTransporter() {
  ...
  if (!user || !pass) {
    ...
    console.warn('EMAIL: SMTP credentials not found (EMAIL_USER/EMAIL_PASS or SMTP_USER/SMTP_PASS). Email disabled until configured.');
    transporter = null;
  ...
  transporter.verify()
    .then(() => {
      console.log('EMAIL: SMTP transporter verified and ready to send messages.');
    })
    .catch((err) => {
...
async function sendEmail({ to, subject, text, html, from }) {
  ...
  if (!t) {
    const err = new Error('SMTP credentials not configured. Set EMAIL_USER/EMAIL_PASS or SMTP_USER/SMTP_PASS in .env');
