...
const laporanLayananService = {
  async getById(id) {
    ...
    if (!laporanLayanan)
      return new ApiError(404, "Laporan kegiatan tidak ditemukan!");
  ...
  async create(dataRaw, file, user) {
    ...
    if (layanan.pemohon.id !== user.id) {
      throw new ApiError(
        403,
        "Akses ditolak! hanya user yang bersangkutan yang dapat mengupload!"
      );
    }

    // PERBAIKAN: Kunjungan & Undangan Narasumber tidak butuh tahap pelaksanaan selesai
    const jenisLayanan =
      layanan.jenis_layanan?.nama_jenis_layanan?.toLowerCase() || "";
    const isSimpleFlow =
      jenisLayanan.includes("kunjungan") ||
    ...
    if (isSimpleFlow) {
      ...
      // untuk PKL/Magang/Pelatihan tetap butuh pelaksanaan selesai
      if (layanan.pelaksanaan.id !== STATUS.SELESAI.id) {
        throw new ApiError(
          409,
          "Pelaksanaan layanan belum selesai! tidak bisa mengirim laporan"
        );
      }
    }
    // kalau sudah pernah selesai laporannya maka tidak bisa submit lagi
    if (
      layanan.laporan.nama_status_kode !==
      STATUS.BELUM_TERSEDIA.nama_status_kode
    ) {
      throw new ApiError(
        409,
        "Laporan sudah pernah dikirim! tidak bisa mengirim laporan lagi"
      );
    }
    // upload file
    ...
    if (!uploadedFoto) {
      throw new ApiError(500, "Terjadi kesalahan ketika upload file");
    }
